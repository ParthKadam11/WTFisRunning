...
func printHelp() {
	fmt.Fprint(os.Stderr, `wtf — see what the fuck is actually running

USAGE
  wtf                      interactive TUI (local)
  wtf user@server          interactive TUI via SSH
  wtf user@server --once   human-readable snapshot
  wtf user@server --json   JSON runtime model
